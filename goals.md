...
- M6-3: タスクテンプレート ✅
  - templates/*.md（new_feature / bug_fix / refactor / new_streamlit_app / api_endpoint / write_tests）
  - src/template_loader.py（list_templates / load_template / render_template）
  - Streamlit タスク投入ページにテンプレート選択 UI を追加
...
### M9: 推論エンジン最適化（Ollama設定）✅（2026-04-06）
- #2: コンテキスト長明示設定（14b=32768 / 7b=8192）✅
- #4: OLLAMA_NUM_PARALLEL=3（ディベート3並列安定化）✅
- #17: KVキャッシュ / keep_alive=-1（バッチロード時間削減）✅
  - src/model_manager.py（unload_model / unload_all_models / get_loaded_models）
  - batch_runner.py の finally ブロックでバッチ終了時アンロード（昼間メモリ解放）
  - coder_agent: complexity ≥ threshold かつ 14b 使用時に deepseek のため自動アンロード
  - scripts/unload_models.sh（手動アンロード用）
  - com.ollama.service.plist に OLLAMA_NUM_PARALLEL / KEEP_ALIVE / NUM_CTX を永続化
